<?php 
  ...
  $id = $_GET['id'];
  $fget = mysqli_query($con, "SELECT * FROM users WHERE id = '$id'");
  $ffetch = mysqli_fetch_array($fget);

  $fname = $ffetch['names'];
  $funame = $ffetch['username'];
  $fpw = $ffetch['password'];
  $fgender = $ffetch['gender'];
  $femail = $ffetch['email'];
  $fage = $ffetch['age'];
  $fimg = $ffetch['profile'];
  ...
  if(isset($_POST['update'])){
    ...
    if($img['size'] == 0){
      $img = 'avatar.jpg';
      $qry = mysqli_query($con, "UPDATE users SET names='$name', username='$uname', gender='$gender', email='$email', age ='$age', `password`='$pw', `profile`='$fimg' WHERE id = '$id'");
      if($qry) {
        header('location: ./');
      }
    } else {
      ...
      if($move){
        $qry = mysqli_query($con, "UPDATE users SET names='$name', username='$uname', gender='$gender', email='$email', age ='$age', `password`='$pw', `profile`='$imgname' WHERE id = '$id'");
        if($qry) {
          header('location: ./');
        }
      }
                ...
                <div class="parts-img">
                  <div class="crd-image">
                    <label id="profilepreview" for="profile">
                      <img src="../img/profiles/users/<?php echo $fimg?>" alt="" id="imagePrev">
